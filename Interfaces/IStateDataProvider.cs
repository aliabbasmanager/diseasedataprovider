...
namespace DiseaseDataProvider.Interfaces
{
    public interface IStateDataProvider
    {
        ...
        int get_total_death_count_by_date(DateTime date = default(DateTime), string state_name = "");
        int get_total_cured_count_by_date(DateTime date = default(DateTime), string state_name = "");
        Dictionary<string, string> get_historical_case_data_all_states();
    }
}